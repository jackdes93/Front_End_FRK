    ...
    <div class="container flex-box bg-food" v-if="brands.length != 0">
        <div class="wrap-container block-online-brands">
            <div class="block-drink-left">
                <p class="text-uppercase title-block-drink">{{$t('hotpot')}}</p>
...
<style scoped>
.fa-heart, .fa-eye, .fa-phone { color: #FFF;}
.bg-food { background: url('../../assets/image/Hotpot.jpg') no-repeat; background-position: center; background-size: cover;}
</style>
    ...
    methods: {
        ...
        handle_get_brand() {
             return new Promise((resolve, reject) => {
                HTTP.get('recommends/brand-exhibition=Hot Pot').then(result => {
                    resolve(result)
