<template>
    <div class="container flex-box bg-drink" v-if="brands.length != 0">
        <div class="wrap-container block-online-brands">
            <div class="block-drink-left">
                <p class="text-uppercase title-block-drink">{{$t('drink')}}</p>
                <a class="title-see-more see-more-desktop" v-on:click="handle_click()">{{$t('see_more')}} >> </a>
            </div>
            <div class="block-drink-brands">
                <div class="brand-item exhibiton-background" style="margin-right: 16px;" v-for="(brand, index) in brands" :key="index">
...
<style>
.bg-drink { background: url('../../assets/image/Drink.jpg') no-repeat; background-position: center; background-size: cover;}
</style>
    ...
    methods: {
        ...
        handle_get_brand() {
             return new Promise((resolve, reject) => {
                HTTP.get('recommends/brand-exhibition=Drink').then(result => {
                    resolve(result)
